...
| `processus` | `crates/processus/src/manifest.json` | 11 | All manifest routes handled by `Processus::dispatch`: shell, capture, detached spawn, env, cwd, pid, args, exit. | Manifest route count; capture shape; shell stdout; cancellation and descendant termination tests. | Covered locally. |
